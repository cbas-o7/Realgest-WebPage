...
async function loadStudentsFilter() {
  ...
  studentSelect.innerHTML = '<option value="all" selected>Todos los Alumnos</option>';
 ...
 */
generateBtn.addEventListener('click', async (e) => { // <--- ¡ARREGLO! Escucha el 'click'
  e.preventDefault();
...
function exportToCSV() {
  const { stats, gestures } = currentReportData;
  if (stats.length === 0 && gestures.length === 0) {
